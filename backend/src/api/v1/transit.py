...
from src.schemas.transit import (
    AggregatedTransitDateResponse,
    DateResponseOrdering,
    TransitDataDateMode,
    TransitResponse,
...
async def dates(
    window_length: int,
    ordering: DateResponseOrdering = Query(DateResponseOrdering.day_asc),
    ...
    return handle_available_dates(db, window_length, ordering)

@router.get("/aggregated/window/{window_length}/start/{start_timestamp}", response_model=list[AggregatedTransitResponse])
async def aggregated_period(
    window_length: int,
    start_timestamp: datetime,
    mode: TransitDataDateMode = Query(TransitDataDateMode.strict),
    db: Session = Depends(get_db)
):
    """
    ...
    return handle_aggregated_data_for_date(db, window_length, start_timestamp, mode)
